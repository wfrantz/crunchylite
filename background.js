...
function redirectEpisodePageRequests(details) {
	var mediaIdMatch = details.url.match(
		/^http:\/\/www\.crunchyroll\.com\/[a-z0-9-]+\/[a-z0-9-]*-(\d{6,})(\?t=\d+)?$/);

	if (mediaIdMatch) {
		var timeCode = 0;
		// Query param t is used when the page URL is updated to 'catch up' with autoplay mode
		// after exiting fullscreen.
		if (mediaIdMatch[2]) {
			timeCode = mediaIdMatch[2].substring(3);
			// Exiting fullscreen with the recommended videos view showing (resulting in t=0, for
			// some reason) should not cause the episode you just watched to start playing again...
			// stupidly high timecode will make the player jump directly to the recommended videos
			// view.
...
function forceAutoAdvanceEnabled(details) {
	...
	if (!details.url.endsWith(queryParamToAppend)) {
		return { redirectUrl: details.url + queryParamToAppend };
	}
	else return {};
}
...
function triggerNextEpisodeDialog(details) {
	getLocalOption('showNextEpisodeDialog', function (optionValue) {
		...
		nextEpisodeRequest.open('GET',
			'http://www.crunchyroll.com/xml/?req=RpcApiVideoPlayer_GetAutoAdvanceTarget&media_id=' +
			mediaIdMatch[1]);
		nextEpisodeRequest.addEventListener('load', function () {
			var nextMediaIdMatch = this.responseText.match(/<url>.*-(\d+)<\/url>/);
			if (nextMediaIdMatch) {
				chrome.tabs.sendMessage(details.tabId, {nextMediaId: nextMediaIdMatch[1]});	
			} 
		});
